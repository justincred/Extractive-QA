...
def build_preprocess_fn(tokenizer, max_length=384, stride=128):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of model names to preprocess for.",
    )
    ...
    for model_name in model_names:
        print(f"\n Preprocessing for model: {model_name}")
        tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=True)

        preprocess_fn = build_preprocess_fn(tokenizer)

        tokenized_train = train_dataset.map(preprocess_fn, batched=True, remove_columns=["id", "context", "question", "answers"])
        tokenized_val   = internal_val_dataset.map(preprocess_fn, batched=True, remove_columns=["id", "context", "question", "answers"])

        model_tag = safe_name(model_name)

        train_path = os.path.join("data", model_tag, "tokenized_squad_train")
        val_path = os.path.join("data", model_tag, "tokenized_squad_internal_val")
